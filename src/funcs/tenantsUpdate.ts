...
export async function tenantsUpdate(
  ...
  Result<
    components.UpdateTenantResponseDto,
    | SDKError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.TenantControllerUpdateTenantRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.UpdateTenantRequestDto, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/tenants/{identifier}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.UpdateTenantResponseDto$inboundSchema),
