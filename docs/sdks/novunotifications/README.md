...
async function run() {
  const result = await novu.subscribers.notifications.unseenCount("<id>", false, 2979.49);
...
async function run() {
  const res = await subscribersNotificationsUnseenCount(novu, "<id>", false, 2979.49);
